...
namespace AdventureGame.Main
{
    public class MenuComponent : DrawableGameComponent
    {
        ...
        protected Color _hilightColor = Color.Red;
        ...
        public void DrawMenuItems()
        {
            ...
            for (int i = 0; i < _menuItems.Count; i++)
            {
                if (i == _selectedItem)
                { 
                    _spriteBatch.DrawString(_hilightFont, _menuItems[i], tempPosition, _hilightColor);
                    tempPosition.Y += _hilightFont.LineSpacing;
                }
                else
                { 
